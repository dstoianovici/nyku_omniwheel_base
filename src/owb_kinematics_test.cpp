...
std::vector<std::vector<float>> read_test_data(const char* file_name){
    ...
    //file.open("/home/Stoianovici/catkin_ws/src/nyku_omniwheel_base/testing_data/goal_pos_test_roll.csv", std::fstream::in);
    ...
    if(file.is_open() == 1){
        while(std::getline(file, line)){
            ...
            while(std::getline(ss,rad_s,',')){
                ...
                float rad_f = std::atof(rad_s.c_str());
                std::cout<<rad_f<<std::endl;
                goal_rpy[j] = rad_f;
                j++;
            };
    ...
    else std::cout << "Could not open poot file"  << std::endl;
